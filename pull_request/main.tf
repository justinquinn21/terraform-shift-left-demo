locals {
  create_kms_key = (
    (var.sns_topic_encryption_enabled ? local.create_sns_topic && length(var.sns_topic_encryption_key_arn) == 0 : false)
    || (var.sqs_encryption_enabled ? length(var.sqs_encryption_key_arn) == 0 : false)
  )
  create_s3_notification = var.integration_type == "S3" && ((var.s3_notification_type == "SNS" && length(var.sns_topic_arn) == 0) || var.s3_notification_type == "SQS")
  create_sns_topic       = local.use_sns_topic ? length(var.sns_topic_arn) == 0 : false
  ...
  wiz_access_via_user  = split("/", split(":", var.wiz_access_role_arn)[5])[0] == "user" ? true : false
  wiz_access_role_list = split("/", var.wiz_access_role_arn)
  wiz_access_role_name = element(local.wiz_access_role_list, length(local.wiz_access_role_list) - 1)
}
...
resource "aws_sns_topic" "wiz-cloud-events" {
  ...
  name              = "wiz-cloudtrail-logs-notify-${random_id.uniq.hex}"
...
resource "aws_sqs_queue" "wiz-cloud-events" {
  name              = "wiz-cloudtrail-logs-queue"
...
resource "aws_iam_policy" "wiz_allow_cloudtrail_bucket_access" {
  name        = "WizAllowCloudTrailBucketAccessPolicy-${random_id.uniq.hex}"
